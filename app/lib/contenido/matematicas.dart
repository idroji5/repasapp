...
Cuerpo _suma(int a, int b) => Cuerpo(
      '$a + $b',
...
Cuerpo _resta(int a, int b) => Cuerpo(
      '$a - $b',
...
Cuerpo _multiplicacion(int a, int b) => Cuerpo(
      '$a × $b',
...
Cuerpo _division(int dividendo, int divisor) {
...
final Map<String, Plantilla> _plantillasDeOperaciones = {
  'suma_2cifras': (a, n) =>
      _suma(a.entre(11, techo(n, 29, 89)), a.entre(11, techo(n, 19, 89))),

  'resta_2cifras': (a, n) {
    final x = a.entre(30, techo(n, 59, 99));
  ...
  'suma_llevada': (a, n) {
    ...
    return _suma(
      a.entre(1, techo(n, 3, 8)) * 10 + ua,
      a.entre(1, techo(n, 2, 8)) * 10 + ub,
    );
  },
  ...
  'resta_llevada': (a, n) {
    ...
    final da = a.entre(2, techo(n, 4, 9));
    final db = a.entre(1, da - 1);
    return _resta(da * 10 + ua, db * 10 + ub);
  },

  'suma_3cifras': (a, n) =>
      _suma(a.entre(120, techo(n, 399, 899)), a.entre(110, techo(n, 299, 799))),

  'resta_3cifras': (a, n) {
    final x = a.entre(250, techo(n, 599, 999));
  ...
  'mult_2x1': (a, n) =>
      _multiplicacion(a.entre(techo(n, 12, 41), techo(n, 39, 99)), a.entre(techo(n, 3, 6), 9)),
  ...
  'mult_3x2': (a, n) => _multiplicacion(
        a.entre(techo(n, 110, 401), techo(n, 399, 999)),
        a.entre(techo(n, 11, 41), techo(n, 29, 99)),
      ),

  'div_exacta_1cifra': (a, n) {
    final d = a.entre(2, 9);
    // Se construye desde el cociente para garantizar que sea exacta.
    return _division(d * a.entre(11, techo(n, 39, 99)), d);
  },

  'div_resto_1cifra': (a, n) {
    final d = a.entre(3, 9);
    return _division(a.entre(20, techo(n, 49, 110)) * d + a.entre(1, d - 1), d);
  },

  'div_2cifras': (a, n) {
    final d = a.entre(techo(n, 12, 21), techo(n, 19, 35));
    return _division(a.entre(techo(n, 11, 31), techo(n, 39, 99)) * d + a.entre(0, d - 1), d);
  },

  'decimales_suma_resta': (a, n) {
    final x = a.entre(15, techo(n, 99, 320)) / 10;
    final y = a.entre(12, techo(n, 59, 199)) / 10;
...
/// Todo lo que se sabe generar: cuentas sueltas y problemas con enunciado.
final Map<String, Plantilla> _plantillas = {
  ..._plantillasDeOperaciones,
  ...plantillasDeProblemas,
};
